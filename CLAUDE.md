...
- `wordbank-sources/` —— **只读参考**：三个第三方词库的 git clone（`ad_checker` / `sensitive-word` / `wordscheck`），是词单的原始来源，不要改这里的代码。
...
它**不再被任何网页引用**（应用壳已删），现在是纯数据资产。要更新词条直接改这里；要扩充词源去 `wordbank-sources/` 提取后并进来。
...
`.gitignore` 排除：
- `wordbank-sources/` —— 三个子目录各自是独立 git clone，自带版本管理，不重复跟踪。
